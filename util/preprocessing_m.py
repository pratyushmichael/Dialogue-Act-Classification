...
def readEmbeddings(embeddingsPath, datasetFiles, frequencyThresholdUnknownTokens, reducePretrainedEmbeddings):
    ...
    neededVocab = {}
    if reducePretrainedEmbeddings:  #false(here)
        ...
        for dataset in datasetFiles:
            dataColumnsIdx = {y: x for x, y in dataset['cols'].items()}   ###??? - x=0,y=tokens
            tokenIdx = dataColumnsIdx['tokens']                             ### tokenIdx = 0
    ...
    for line in embeddingsIn:
        ...
        if len(word2Idx) == 0:  # Add padding+unknown
            word2Idx["PADDING_TOKEN"] = len(word2Idx)   ###word2Idx["PADDING_TOKEN"] = 0 -dictionary
            vector = np.zeros(embeddingsDimension)
            embeddings.append(vector)

            word2Idx["UNKNOWN_TOKEN"] = len(word2Idx)     ### word2Idx["UNKNOWN_TOKEN"] = 1
            vector = np.random.uniform(-0.25, 0.25, embeddingsDimension)  # Alternativ -sqrt(3/dim) ... sqrt(3/dim)
            embeddings.append(vector)

        vector = np.array([float(num) for num in split[1:]]) ###contains numerical part of embedding file
    ...
    def createFD(filename, tokenIndex, fd, word2Idx):
        for line in open(filename):
            ...
            if len(splits) > 1:
                word = splits[tokenIndex]      ### splits[0]
                wordLower = word.lower()
                wordNormalized = wordNormalize(wordLower)
    ...
    if frequencyThresholdUnknownTokens != None and frequencyThresholdUnknownTokens >= 0:
        ...
        for datasetName, datasetFile in datasetFiles.items():   
            dataColumnsIdx = {y: x for x, y in datasetFile['columns'].items()}  #{0: 'tokens', 1: 'POS'}
            tokenIdx = dataColumnsIdx['tokens']
            datasetPath = 'data/%s/' % datasetName
            createFD(datasetPath + 'train.txt', tokenIdx, fd, word2Idx)

        addedWords = 0
        for word, freq in fd.most_common(10000):                    ###10000 most common words
...
def addCharInformation(sentences):
    ...
    for sentenceIdx in range(len(sentences)):                           ###sentences = [{},{},.....]
        sentences[sentenceIdx]['characters'] = []                       ###sentenceIdx = {'characters':[],...}
        for tokenIdx in range(len(sentences[sentenceIdx]['tokens'])):    ###???
            ...
            #for t in range(len(sentences[sentenceIdx]['tokens'][tokenIdx])):
               #token = sentences[sentenceIdx]['tokens'][tokenIdx][t]       ##sentenceIdx = {'tokens':[],...}
               #chars = [c for c in token]                                  ###???
               #sentences[sentenceIdx]['characters'].append(chars)          ###???
...
def addCasingInformation(sentences):
    ...
    for sentenceIdx in range(len(sentences)):
        sentences[sentenceIdx]['casing'] = []                   ###sentenceIdx= {'casing':[],...}
        for tokenIdx in range(len(sentences[sentenceIdx]['tokens'])):  ##sentenceIdx = {'tokens':[],...}
            token = sentences[sentenceIdx]['tokens'][tokenIdx]
            sentences[sentenceIdx]['casing'].append(getCasing(token))
            #for t in range(len(sentences[sentenceIdx]['tokens'][tokenIdx])):
                #token = sentences[sentenceIdx]['tokens'][tokenIdx][t]
                #sentences[sentenceIdx]['casing'].append(getCasing(token))
...
###{'numeric': 2, 'other': 1, 'mainly_numeric': 3, 'PADDING': 0,......}


def createMatrices(sentences, mappings, padOneTokenSentence):   ###sentences = [{},{},.....]
    data = []                                                   ###characters,casings,tokens,POS - sentences    
    numTokens = 0
    numUnknownTokens = 0    
    ...
    for sentence in sentences:
        row = {name: [] for name in list(mappings.keys())+['raw_tokens']}###{'tokens': [], 'casing': [], 'raw_tokens': [],'POS':[],'characters':[]}
        
        for mapping, str2Idx in mappings.items():   ###  mappings = {'tokens': word2Idx, 'casing': casing2Idx}

            if mapping not in sentence:             ###
                continue
                    
            for entry in sentence[mapping]:             ###tokens,casing =sentence[mapping] -lists[]  
                if mapping.lower() == 'tokens':         ###iteration for every list member of - tokens,....
                     for en in range(len(sentence['tokens'])):
                        zx = sentence['tokens'][en]
                        zx = ast.literal_eval(zx)
                        for j in range(len(zx)):
                          z = zx[j]
                          numTokens += 1
                          idx = str2Idx['UNKNOWN_TOKEN']  ###    word2Idx = {}
                    
                          if z in str2Idx:
                                 idx = str2Idx[z]
                          elif z.lower() in str2Idx:
                                 idx = str2Idx[z.lower()]
                          elif wordNormalize(z) in str2Idx:
                                 idx = str2Idx[wordNormalize(z)]
                          else:
                                 numUnknownTokens += 1    
                                 missingTokens[wordNormalize(z)] += 1
                        
                          row['raw_tokens'].append(z)
                elif mapping.lower() == 'characters':  
                    idx = []
                    for c in entry:                 ###for every char c of entry
                        if c in str2Idx:
...
def extendMappings(mappings, sentences):        ##sentences = [{}, {},...]- each dictionary -sentence
    sentenceKeys = list(sentences[0].keys())    ###tokens,POS - sentences 
    sentenceKeys.remove('tokens') #No need to map tokens

    for sentence in sentences:
        for name in sentenceKeys:     
            if name not in mappings:         ###  mappings = {'tokens': word2Idx, 'casing': casing2Idx, POS:...,characters:...}
                mappings[name] = {'O':0} #'O' is also used for padding   ###???

            for item in sentence[name]:              
                if item not in mappings[name]:
                    mappings[name][item] = len(mappings[name])      ###add in mappings[name]
