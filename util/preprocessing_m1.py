...
from .CoNLL_m1 import readCoNLL
...
def readEmbeddings(embeddingsPath, datasetFiles, frequencyThresholdUnknownTokens, reducePretrainedEmbeddings):
    ...
    if reducePretrainedEmbeddings:
        ...
        for dataset in datasetFiles:
            dataColumnsIdx = {y: x for x, y in dataset['cols'].items()}   ###??? - x=1,y=tokens
            tokenIdx = dataColumnsIdx['tokens']                             ### tokenIdx = 1
            print(tokenIdx)
    ...
    def createFD(filename, tokenIndex, fd, word2Idx):
        for line in open(filename):
            ...
            if len(splits) > 1:
                word = splits[tokenIndex]      ### splits[1]
                wordLower = word.lower()
                wordNormalized = wordNormalize(wordLower)
...
def addCharInformation(sentences):
    ...
    for sentenceIdx in range(len(sentences)):                           ###sentences = [{},{},.....]
        ...
        for tokenIdx in range(len(sentences[sentenceIdx]['tokens'])):    ###???
            token = sentences[sentenceIdx]['tokens'][tokenIdx]       ##sentenceIdx = {'tokens':[],...}
...
def createMatrices(sentences, mappings, padOneTokenSentence):   ###sentences = [{},{},.....]
    data = []                                                   ###characters,casings,tokens - sentences    
    numTokens = 0
    numUnknownTokens = 0    
    ...
    for sentence in sentences:
        row = {name: [] for name in list(mappings.keys())+['raw_tokens']}###{'tokens': [], 'casing': [], 'raw_tokens': []}
...
def extendMappings(mappings, sentences):
    sentenceKeys = list(sentences[0].keys())    ###characters,casings,tokens - sentences 
    sentenceKeys.remove('tokens') #No need to map tokens

    for sentence in sentences:
        for name in sentenceKeys:     
            if name not in mappings:         ###  mappings = {'tokens': word2Idx, 'casing': casing2Idx}
